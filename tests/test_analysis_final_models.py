...
def test_load_pixel_models_orders_by_features_and_skips_field_level(tmp_path):
    ...
    old = {k: v for k, v in base.items() if k != "threshold"} | {"columns": COLS[:6], "threshold_maize": 0.5}
    joblib.dump(old, tmp_path / "xgb_old_name.joblib")       # saved before the threshold key was renamed
    b = fm.load_pixel_models(tmp_path)
    assert [x["file"] for x in b] == ["xgb_full.joblib", "xgb_old_name.joblib", "xgb_only.joblib"]
    assert b[1]["threshold"] == 0.5
