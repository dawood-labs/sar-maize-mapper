...
|---|---|---|
| **Structure / roughness** | Size, shape and orientation of the things the wave hits, compared to the 5.5 cm wavelength | Tall maize stalks, leaves, a ploughed field vs a smooth one |
...
> The real values for the R&D AOI will come from the ground-truth polygons. Use these only to build
> intuition and to spot obviously wrong outputs.
...
growth, a sharp drop at harvest.

### Rice

- Fields are **flooded** at transplanting, so the water mirror makes **both VV and VH very low**
  (often below −20 dB in VH). This "flood dip" is rice's strongest fingerprint.
- As plants grow out of the water, stems + water create double bounce and VV and VH rise quickly.
- Harvest → drop.

### Sugarcane (≈10–12 months)

- Very dense, tall canopy for **many months**, so **VH stays high** (≈ −14 to −11 dB) for a long plateau.
- The length of the high period separates it from maize (~4 months).
...
- Maize lasts ~100–120 days, and key transitions (emergence, peak, harvest) take only a few weeks.
