...
```bash
git clone git@github.com:<org>/sar-maize-mapper.git
cd sar-maize-mapper
...
data/gcp/    ground-truth polygons (later)
...
python -m ipykernel install --user --name sar-maize-mapper
```

Then choose the `sar-maize-mapper` kernel in the notebooks.
