...
- For **classification** (is this pixel maize?) a scale difference of ~1 % is irrelevant.
- For **area statistics** (hectares from pixel counts), apply a per-location scale-factor correction far from the
