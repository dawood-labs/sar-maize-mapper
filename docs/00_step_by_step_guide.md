...
7. [Part C — Check the ground truth](#7-part-c--check-the-ground-truth)
...
| Part | What happens | Where it runs | Typical time |
|---|---|---|---|
| A | Google Earth Engine cleans the radar images and saves them to cloud storage; you download them | Earth Engine + your machine | hours (export), minutes to an hour (download) |
| B | Plot how the radar signal of one spot changed through the season | your machine | seconds |
| C | Check the labelled fields (ground truth) for mistakes | your machine | minutes |
| D | Train and test the crop model | your machine | 30–90 minutes |
| E | Classify every pixel of an area | your machine | minutes |
| F | Add polygons where the map is wrong, retrain, map again | your machine | as D + E |
| G | Give every field polygon a crop label | your machine | minutes |
...
| **Ground truth / GCP polygons** | Fields someone checked and labelled with their real crop. The model learns from these. |
...
| **F1** | One score between 0 and 1 that combines "how many maize pixels were found" (recall) and "how many pixels called maize really are maize" (precision). 0.90 is good. |
| **Threshold** | The maize probability a pixel needs before it is called maize (e.g. 0.75). |
...
1. **Always start in the project folder** and with the Python environment switched on:
   ```bash
   cd ~/sar-maize-mapper          # or wherever you cloned it
...
```bash
git clone git@github.com:<account>/sar-maize-mapper.git
cd sar-maize-mapper
python3 -m venv .venv
...
| `analysis.target_class` | the crop of interest, spelled exactly as in the ground truth, e.g. `Maize` |
...
### 5.3 Audit: which satellite passes exist?
...
### 5.5 Pilot export (one chunk first)
...
- **VH rises for weeks, then falls:** a crop growing and being harvested. About 4 months → maize; 10–12 months →
  sugarcane.
...
3. Picks the maize probability threshold.
...
| File | Content |
|---|---|
| `<map_name>_classes.tif` (+ `.qml`) | crop class, **main map** (maize when its probability ≥ the threshold) |
| `<map_name>_classes_argmax.tif` | crop class with the highest probability, for comparison |
| `<map_name>_maize_prob.tif` | maize probability 0–100 % (255 = no data) |
...
3. Drag in `<map_name>_maize_prob.tif`: *Properties → Symbology → Singleband pseudocolor*, min 0, max 100.
...
nohup python -m sar_pipeline.analysis field-labels --config $MAP \
    ...
    --prob-raster  $A/<map_name>_maize_prob.tif \
...
|---|---|---|
| `majority_class` | the label | `"majority_class" = 'Maize'` |
...
| `AuditRequired` | no tracks chosen | fill `s1.tracks` (5.3) |
...
| `DecisionRequired` from `stack` | missing dates or chunks | read `decisions_required.md` (5.8) |
...
python -m sar_pipeline.analysis field-labels --config <area config> --delineation <fields file> \
    --class-raster <map>_classes.tif --prob-raster <map>_maize_prob.tif \
    [--model <final_models>/xgb_field_level.joblib --train-config <training config> --train-run <run>] --name <fields_name>

# any command: add --run <run id> to use an older run instead of the newest
